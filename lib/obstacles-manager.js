'use strict'

const Matter = require('matter-js')

const Composite = Matter.Composite

const Press = require('./obstacles/press')
const SimpleHole = require('./obstacles/simple-hole')
const SpaceInvaders = require('./obstacles/space-invaders')

const { EventEmitter } = require('events')

class ObstaclesManager {
  constructor(game) {
    this.obstacles = []
    this.level = 0
    this.game = game
    this.compositeObj = Matter.Composite.create({ label: 'obstacles' })
    this.eventEmitter = new EventEmitter()

    this.walls = Composite.create({ label: 'walls' })
    this.initWalls()
  }

  initWalls() {
    Matter.Composite.add(this.walls, [
      // Top
      Matter.Bodies.rectangle(canvasWidth / 2, -5, canvasWidth, 10, {
        isStatic: true,
      }),
      // Left
      Matter.Bodies.rectangle(-5, canvasWidth / 2, 10, canvasWidth, {
        isStatic: true,
      }),
      // Right
      Matter.Bodies.rectangle(
        canvasWidth + 5,
        canvasWidth / 2,
        10,
        canvasWidth,
        {
          isStatic: true,
        },
      ),
      // Bottom
      Matter.Bodies.rectangle(
        canvasWidth / 2,
        canvasWidth + 5,
        canvasWidth,
        10,
        { isStatic: true },
      ),
    ])
  }

  getGame() {
    return this.game
  }

  getComposite() {
    return this.compositeObj
  }

  getWallsComposite() {
    return this.walls
  }

  getObstacles() {
    return this.obstacles
  }

  resetObstacles() {
    this.obstacles = []
  }

  getObstaclesParts() {
    let parts = []
    _.forEach(this.getObstacles(), (obstacle, key) => {
      parts = _.concat(parts, obstacle.getVertices())
    })

    return parts
  }

  getStartLevel() {
    return this.startLevel
  }

  getLevel() {
    return this.level
  }

  getDynamicLevel() {
    return this.getLevel() + this.getStartLevel()
  }

  getEventEmmitter() {
    return this.eventEmitter
  }

  setStartLevel(level) {
    this.startLevel = level
  }
  setLevel(level) {
    this.level = level
  }

  initObstacle(params = {}) {
    let obstacleID = helpers.getRandomInt(2, 3)
    params.level = this.getDynamicLevel()

    let newObstacle
    switch (obstacleID) {
      case 1:
        newObstacle = new SpaceInvaders(params)
        break
      case 2:
        newObstacle = new SimpleHole(params)
        break
      case 3:
        newObstacle = new Press(params)
        break
      default:
        break
    }

    newObstacle.getEventEmmitter().on('obstaclePartOver', (obstaclePartID) => {
      Matter.Composite.remove(newObstacle.getComposite(), obstaclePartID)
    })

    this.obstacles.push(newObstacle)

    Matter.Composite.add(this.getComposite(), newObstacle.getComposite())
  }

  updateObstacles() {
    _.forEach(this.getObstacles(), (obstacle, obstacleKey) => {
      obstacle.loop()
      if (obstacle.isOver()) {
        Matter.Composite.remove(this.getComposite(), obstacle.getComposite())
        this.getEventEmmitter().emit('obstacleOver', {})
        delete this.obstacles[obstacleKey]
        this.obstacles = this.obstacles.filter(Boolean)
      }
    })
  }
}

module.exports = ObstaclesManager
