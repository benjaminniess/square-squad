...
class ObstaclesManager {
    ...
    initWalls() {
        ...
        Matter.Composite.add(this.walls, [
            // Top
            Matter.Bodies.rectangle(canvasWidth / 2, -5, canvasWidth, 10, commonProperties),
            // Left
            Matter.Bodies.rectangle(-5, canvasWidth / 2, 10, canvasWidth, commonProperties),
            // Right
            Matter.Bodies.rectangle(canvasWidth + 5, canvasWidth / 2, 10, canvasWidth, commonProperties),
            // Bottom
            Matter.Bodies.rectangle(canvasWidth / 2, canvasWidth + 5, canvasWidth, 10, commonProperties)
        ]);
    }
    getGame() {
        return this.game;
    ...
    initObstacle(params = {}) {
        ...
        newObstacle
            .getEventEmmitter()
            .on('obstaclePartOver', (obstaclePart) => {
    ...
    updateObstacles() {
        _.forEach(this.getObstacles(), (obstacle, obstacleKey) => {
            obstacle.loop();
            if (obstacle.isOver()) {
                Matter.Composite.remove(this.getComposite(), obstacle.getComposite(), true);
