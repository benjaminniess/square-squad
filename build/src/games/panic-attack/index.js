...
class Panick_Attack extends MasterGame {
        ...
        Matter.Events.on(this.getEngine(), 'collisionStart', (event) => {
            _.forEach(event.pairs, (collisionPair) => {
                if (collisionPair.bodyA.enableCustomCollisionManagement === true) {
                    let targetObstacle = this.getObstaclesManager().getObstacleFromBodyID(collisionPair.bodyA.id);
                    if (targetObstacle) {
                        targetObstacle.onCollisionStart(collisionPair.bodyA, collisionPair.bodyB);
                    }
                }
                if (collisionPair.bodyB.enableCustomCollisionManagement === true) {
                    let targetObstacle = this.getObstaclesManager().getObstacleFromBodyID(collisionPair.bodyB.id);
                    if (targetObstacle) {
                        targetObstacle.onCollisionStart(collisionPair.bodyB, collisionPair.bodyA);
                    }
                }
                let player;
                let otherBody;
                ...
                Matter.Composite.remove(this.getPlayersManager().getComposite(), player, true);
            ...
            Matter.Events.on(this.getEngine(), 'collisionEnd', (event) => {
                _.forEach(event.pairs, (collisionPair) => {
                    if (collisionPair.bodyA.enableCustomCollisionManagement === true) {
                        let targetObstacle = this.getObstaclesManager().getObstacleFromBodyID(collisionPair.bodyA.id);
                        if (targetObstacle) {
                            targetObstacle.onCollisionEnd(collisionPair.bodyA, collisionPair.bodyB);
                        }
                    }
                    if (collisionPair.bodyB.enableCustomCollisionManagement === true) {
                        let targetObstacle = this.getObstaclesManager().getObstacleFromBodyID(collisionPair.bodyB.id);
                        if (targetObstacle) {
                            targetObstacle.onCollisionEnd(collisionPair.bodyB, collisionPair.bodyA);
                        }
                    }
                });
            });
    ...
    refreshData() {
        ...
        if (this.getStatus() === 'playing') {
            if (!process.env.DISABLE_OBSTACLES ||
            ...
            _.forEach(playersManager.getPlayersMoves(), (moves, playerID) => {
                ...
                bonusList.map((bonus) => {
                    ...
                    if (playerData.x - squareSize / 2 < bonusData.x + bonusData.width &&
